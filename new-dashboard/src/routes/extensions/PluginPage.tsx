...
export function isTrustedPluginMessageOrigin(origin: string, expectedOrigin = window.location.origin, lockedOrigin?: string | null) {
...
export default function PluginPage() {
  ...
  const { pluginName = '', pageName = '' } = useParams(); const frame = useRef<HTMLIFrameElement>(null); const messageOrigin = useRef<string | null>(null); const [plugin, setPlugin] = useState<JsonObject>({}); const [page, setPage] = useState<JsonObject>({}); const [src, setSrc] = useState(''); const [loading, setLoading] = useState(true); const [error, setError] = useState('');
  const post = useCallback((payload: JsonObject) => frame.current?.contentWindow?.postMessage({ channel: CHANNEL, ...payload }, pluginMessageTargetOrigin(messageOrigin.current)), []);
  ...
  return <iframe className="plugin-page-frame" onLoad={() => { messageOrigin.current = null; sendContext(); }} ref={frame} referrerPolicy="no-referrer" sandbox="allow-scripts allow-forms allow-downloads" src={src} title={`${pluginName}: ${pageName}`} />;
}
