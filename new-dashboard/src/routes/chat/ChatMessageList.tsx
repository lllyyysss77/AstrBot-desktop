  ...
  return (
    <div className={`ab-chat-message-list ${className}`.trim()}>
      {messages.map((message, index) => {
        ...
        const activeStreaming = !user && streaming
          && (streamingMessageId == null ? index === messages.length - 1 : String(message.id) === String(streamingMessageId));
        ...
        const isEditing = editingMessageId != null && message.id != null && String(editingMessageId) === String(message.id);
        ...
        const canRetry = !user && enableRetry && Boolean(onRetry) && index === messages.length - 1
          && !activeStreaming && Boolean(message.llm_checkpoint_id);
        ...
        return (
          ...
          >
            {!user && (
              <div className={`ab-chat-message-avatar ${activeStreaming ? 'is-streaming' : ''}`} title={labels.assistant}>
            ...
            <div className="ab-chat-message-stack">
              {attachments.length > 0 && (
                <div className={`ab-chat-sent-attachments ${attachments.every((part) => part.type === 'image') ? 'is-images-only' : ''}`}>
                ...
                >
                  {isEditing
                    ? <div className="ab-chat-inline-edit">
                        ...
                        />
                        <div>
                          <button disabled={editSaving} onClick={onCancelEdit} type="button">{labels.cancel}</button>
                          <button className="is-primary" disabled={editSaving || !editingValue.trim()} onClick={onSaveEdit} type="button">{labels.save}</button>
                        </div>
                      </div>
                    : message.content.isLoading && bubbleParts.length === 0
                    ? <span className="ab-chat-message-loading"><span className="ab-chat-message-spinner" /> {labels.running}</span>
              ...
              {showMeta && !isEditing && <div className="ab-chat-message-meta">
                {message.created_at ? <time dateTime={String(message.created_at)}>{formatTime(message.created_at)}</time> : null}
                {canEdit ? <IconButton icon="mdi-pencil-outline" label={labels.edit} onClick={() => onEdit?.(message)} /> : null}
                {canRetry ? <RetryMenu
                  labels={labels}
                  message={message}
                  models={retryModels}
                ...
                {!user && message.content.agentStats ? <StatsMenu labels={labels} stats={message.content.agentStats} /> : null}
                {threads.length > 0 ? (
...
function AstrBotMark() {
  return <svg aria-hidden="true" className="ab-chat-astrbot-mark" viewBox="0 0 24 24">
...
function StatsMenu({ labels, stats }: { labels: ChatMessageLabels; stats: JsonObject }) {
  ...
  const input = statNumber(stats, ['token_usage.input_other', 'token_usage.input_tokens', 'input_tokens', 'prompt_tokens']);
  const output = statNumber(stats, ['token_usage.output', 'token_usage.output_tokens', 'output_tokens', 'completion_tokens']);
  const start = statNumber(stats, ['start_time']);
  const end = statNumber(stats, ['end_time']);
  const duration = statNumber(stats, ['duration', 'total_duration', 'agent_duration']) || (start && end ? end - start : 0);
  const ttft = statNumber(stats, ['ttft', 'time_to_first_token', 'first_token_time']);
  return <details className="ab-chat-stats-menu">
    <summary><MdiIcon name="mdi-information-outline" size={17} /></summary>
    <div className="ab-chat-stats-popover">
      {cached > 0 && <div><span>{labels.cachedTokens}</span><strong>{formatCount(cached)}</strong></div>}
      <div><span>{labels.inputTokens}</span><strong>{formatCount(input)}</strong></div>
      <div><span>{labels.outputTokens}</span><strong>{formatCount(output)}</strong></div>
      {ttft > 0 && <div><span>{labels.ttft}</span><strong>{formatSeconds(ttft)}</strong></div>}
      <div><span>{labels.duration}</span><strong>{formatSeconds(duration)}</strong></div>
    </div>
  </details>;
}
...
function MessagePart(props: MessagePartProps) {
  ...
  if (part.type === 'plain') {
    ...
    return <Fragment>{renderMarkdown?.(content, { message, streaming }) ?? <Markdown content={content} streaming={streaming} />}</Fragment>;
  }
  if (part.type === 'think' || part.type === 'reasoning') {
    const reasoning = String(part.think || part.text || '');
    return <ReasoningPart labels={labels} message={message} onOpen={props.onOpenReasoning} reasoning={reasoning} streaming={streaming} />;
  }
  if (part.type === 'reply') {
    ...
    return (
      <button className="ab-chat-reply-quote" disabled={messageId == null} onClick={() => messageId != null && props.onReplyClick?.(messageId, message)} type="button">
        <MdiIcon name="mdi-reply" size={16} /><span>{preview}</span>
      </button>
    );
  }
...
function ReasoningPart({ labels, message, reasoning, streaming, onOpen }: {
  labels: ChatMessageLabels;
  message: ChatRecord;
  reasoning: string;
  streaming: boolean;
  ...
  return (
    <details className="ab-chat-reasoning" open={streaming}>
      <summary>
        ...
        {onOpen ? (
          <button className="ab-chat-reasoning-open" onClick={(event) => { event.preventDefault(); onOpen(message, reasoning); }} type="button">
  ...
  return (
    <div className="ab-chat-tool-calls">
      {calls.map((tool, index) => {
        const finished = Boolean(tool.finished_ts) || ['done', 'completed', 'success'].includes(String(tool.status || '').toLowerCase());
        return (
          <details className={`ab-chat-tool-call ${finished ? 'is-complete' : 'is-running'}`} key={tool.id || `${tool.name}-${index}`}>
            <summary>
              <span className="ab-chat-tool-icon"><MdiIcon name={isCodeTool(tool.name) ? 'mdi-code-json' : 'mdi-hammer-wrench'} size={17} /></span>
...
function ToolValue({ label, value }: { label: string; value: unknown }) {
  ...
  return <div className="ab-chat-tool-value"><strong>{label}</strong><pre>{typeof value === 'string' ? value : safeJson(value)}</pre></div>;
}

type AttachmentPartProps = Pick<MessagePartProps, 'labels' | 'message' | 'part' | 'resolvePartUrl' | 'onDownload' | 'onOpenImage'> & { sent: boolean };
...
function AttachmentPart({ labels, message, part, resolvePartUrl, onDownload, onOpenImage, sent }: AttachmentPartProps) {
  ...
  if (part.type === 'image') {
    return (
      <button className={`ab-chat-image-part ${sent ? 'is-sent' : ''}`} onClick={() => onOpenImage?.(url, part, message)} type="button">
        <img alt={name} src={url} />
      </button>
    );
  }
  if (part.type === 'record' || part.type === 'audio') return <audio className="ab-chat-audio-part" controls src={url} />;
  ...
  return (
    <div className={`ab-chat-file-part ${sent ? 'is-sent' : ''}`}>
      <span className="ab-chat-file-icon"><MdiIcon name={attachmentIcon(name)} size={25} /></span>
      <span className="ab-chat-file-meta"><strong title={name}>{name}</strong><small>{attachmentExtension(name)}</small></span>
      {onDownload ? <IconButton icon="mdi-download" label={labels.download} onClick={() => onDownload(part, message)} /> : null}
    </div>
  );
}

function RetryMenu({ labels, message, models, onRetry, onRetryWithModel }: {
  labels: ChatMessageLabels;
  message: ChatRecord;
  ...
  return <details className="ab-chat-retry-menu">
    <summary aria-label={labels.retry} title={labels.retry}><MdiIcon name="mdi-refresh" size={17} /></summary>
    <div className="ab-chat-retry-popover">
      <button onClick={() => onRetry?.(message)} type="button"><MdiIcon name="mdi-refresh" size={16} />{labels.retry}</button>
      {onRetryWithModel && models.map((item) => <button
      ...
      >
        ...
        <span><strong>{item.providerId}</strong><small>{item.model}</small></span>
      </button>)}
    </div>
  </details>;
}

function ThreadMenu({ labels, message, onOpenThread, threads }: {
  labels: ChatMessageLabels;
  message: ChatRecord;
  ...
  return (
    <details className="ab-chat-thread-menu">
      <summary className="ab-chat-meta-chip"><MdiIcon name="mdi-source-branch" size={15} />{labels.threads} {threads.length}</summary>
      <div className="ab-chat-thread-popover">
        {threads.map((thread) => (
          <button key={thread.thread_id} onClick={() => onOpenThread?.(thread, message)} type="button">
            <MdiIcon name="mdi-source-branch" size={15} />{truncate(String(thread.selected_text || labels.threads), 48)}
          </button>
        ))}
      </div>
    </details>
...
function IconButton({ icon, label, onClick }: { icon: `mdi-${string}`; label: string; onClick: () => void }) {
  return <button aria-label={label} className="ab-chat-icon-button" onClick={onClick} title={label} type="button"><MdiIcon name={icon} size={17} /></button>;
}
...
function plainText(message: ChatRecord) {
  return messageParts(message).filter((part) => part.type === 'plain').map((part) => String(part.text || '')).join('\n').trim();
}

function messageThreads(message: ChatRecord): ChatThread[] {
  const value = message.threads;
  return Array.isArray(value) ? value.filter((thread): thread is ChatThread => Boolean(thread && typeof thread === 'object' && typeof (thread as JsonObject).thread_id === 'string')) : [];
}
...
function messageReferences(message: ChatRecord): ChatReference[] {
  ...
  return Array.isArray(value) ? value.filter((item): item is ChatReference => Boolean(item && typeof item === 'object')) : [];
}
...
function defaultPartUrl(part: ChatPart) {
  ...
  if (embedded && typeof embedded === 'object' && typeof (embedded as JsonObject).url === 'string') return String((embedded as JsonObject).url);
