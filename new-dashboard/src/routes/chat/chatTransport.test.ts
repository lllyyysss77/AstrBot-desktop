...
describe('chat transport', () => {
  it('parses split SSE frames and flushes a final frame without a delimiter', async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return streamResponse([
        'data: {"type":"plain","data":"hel',
        'lo"}\r\n\r\ndata: {"type":"end"}',
      ]);
    });

    await runChatStream(sendAction, new AbortController().signal, {
      onComplete,
  ...
  it('maps every SSE action to the shared executor without mixing concurrent payloads', async () => {
    ...
    await Promise.all([
      ...
      }, controller.signal, { onPayload: (payload) => regenerated.push(payload) }, { fetch: fetchMock as typeof fetch }),
      runChatStream({
        kind: 'continue',
        configId: 'profile',
        enableStreaming: true,
        ...
        sessionId: 'session-3',
      }, controller.signal, { onPayload: (payload) => continued.push(payload) }, { fetch: fetchMock as typeof fetch }),
      runChatStream({
        kind: 'thread',
        enableStreaming: true,
        message: [{ type: 'plain', text: 'thread' }],
        threadId: 'thread / 4',
      }, controller.signal, { onPayload: (payload) => threaded.push(payload) }, { fetch: fetchMock as typeof fetch }),
    ]);

    expect(requests.map((request) => request.endpoint)).toEqual(expect.arrayContaining([
  ...
  it('maps non-SSE responses to a transport error and emits the error event', async () => {
    ...
    await expect(runChatStream(sendAction, new AbortController().signal, {
      onError,
      onPayload: vi.fn(),
    }, {
  ...
  it('cancels an active reader and emits abort without reporting an error', async () => {
    ...
    const onAbort = vi.fn();
    const onError = vi.fn();
    const running = runChatStream(sendAction, controller.signal, {
      onAbort,
      onError,
      onPayload: vi.fn(),
    }, {
  ...
  it('builds the authenticated WebSocket URL and emits payload and completion events', async () => {
    ...
    await runChatStream({ ...sendAction, transport: 'websocket' }, new AbortController().signal, {
      onComplete,
      onPayload: (payload) => payloads.push(payload),
    }, {
